...
static int32 compare(uint8 *arg1, uint8 *arg2, os_size_t size)
{
	os_size_t i;
	for (i = 0; i < size; i++)
	{
...
static int32 os_multiset_compare(void *key1, void *key2, void *arg)
{
...
{
	os_multiset_iterator *itr = (os_multiset_iterator *)malloc(sizeof(os_multiset_iterator) + obj->set.unit_size);
