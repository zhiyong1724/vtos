...
{
	list_node_type_def *node = (list_node_type_def *)os_malloc(sizeof(list_node_type_def) + obj->unit_size);
...
os_size_t os_list_insert(os_list *obj, void *data, os_size_t n)
{
	...
	else if (n < obj->size)
	{
		...
		node = (list_node_type_def *)os_malloc(sizeof(list_node_type_def) + obj->unit_size);
		os_mem_cpy(&node[1], data, obj->unit_size);
